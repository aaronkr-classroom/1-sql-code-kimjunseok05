...
SELECT 이름, 학년, 소속학과, 휴대폰번호 FROM 학생
WHERE (학년>=1 AND 학년<=3) OR NOT (소속학과='컴퓨터'); -- BETWEEN 1 AND 3도 괜찮아요
...
SELECT * FROM 수강
ORDER BY 중간성적 DESC
LIMIT 3 OFFSET 5; -- LIMIT 5,3은 MYSQL만
...
SELECT 성별, MAX(나이) AS 최고나이, MIN(나이) AS 최저나이 FROM 학생
GROUP BY 성별;

SELECT 나이, COUNT(*) AS 나이별학생수 FROM 학생
WHERE 나이 >= 20 AND 나이 < 30
GROUP BY 나이;

SELECT 학년, COUNT(*) AS 학년별학생수 FROM 학생
GROUP BY 학년
HAVING COUNT(*)>=2;
...
FROM 학생 LEFT OUTER JOIN 수강 ON 학생.학번 = 수강.학번; -- 왼쪽 외부 조인

SELECT 학생.학번, 이름, 평가학점
FROM 학생 RIGHT OUTER JOIN 수강 ON 학생.학번 = 수강.학번; -- 오른쪽 외부 조인


SELECT 학생.학번, 이름, 평가학점
FROM 학생 FULL OUTER JOIN 수강 ON 학생.학번 = 수강.학번; -- 전체 외부 조인
